...
public class UserService {
    ...
    @Transactional
    public void updateUserAccountState(Long id) {
        Query query = new Query(Criteria.where("_id").is(id).and("state").is(PENDING));
        Update update = new Update().set("state", ACTIVE);

        var updateResult = mongoTemplate.updateFirst(query, update, User.class);
        if (updateResult.getModifiedCount() == 0) {
            throw new UserAccountActivationException("User account state update failed. No pending user found with id: " + id);
        }
    }
